...
import { Contact, Account } from "interfaces/profile";

import { addAddress, updateAddress } from "actions/profile";

import { addContactAddress, updateContactAddress } from "request/address";
...
class ContactModal extends Component<Props, State> {
    ...
    sendRequest = async () => {
        ...
        const isEdit = _.get(_data, "id", "") !== "";

        let requestHandler = isEdit ? updateContactAddress : addContactAddress;

        if (isEdit) {
            _.set(item, "id", _.get(_data, "id"));
        }

        _.set(item, "country", _.toUpper(region.country));

        _.set(item, "accountId", account.id);

        const { status, data } = await requestHandler(item);
    ...
    render() {
        const { region, account, data } = this.props;
...
const mapDispatchToProps = (dispatch: any) => {
    return {
        update_address: (data: any) => dispatch(updateAddress(data)),
        add_address: (data: any) => dispatch(addAddress(data)),
    };
};
