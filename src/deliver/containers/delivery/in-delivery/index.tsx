...
import { INDELIVERY_DELIVERIES } from "deliver/graphql/query";
...
const InDeliveryDelivery: FC<Props> = () => {
    const { deliver } = useDeliver();

    const { data, error, loading, networkStatus } = useQueryMore(
        INDELIVERY_DELIVERIES,
        {
            deliver: deliver.id,
        }
    );
...
export default InDeliveryDelivery;
